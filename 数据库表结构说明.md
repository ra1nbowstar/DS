...
- `points` - DECIMAL(12,4) NOT NULL DEFAULT 0.0000（积分，支持4位小数）
...
- `referral_code` - BIGINT UNSIGNED NULL UNIQUE（推荐码，唯一）
...
**字段列表：**
- `id` - BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY（主键）
- `sku` - VARCHAR(64) UNIQUE（SKU编码，唯一）
...
- `is_vip` - TINYINT(1) DEFAULT 0（是否VIP商品）
...
**字段列表：**
- `id` - BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY（主键）
- `order_no` - VARCHAR(64) UNIQUE（订单号，唯一）
- `order_number` - VARCHAR(50) UNIQUE（订单号，兼容字段）
...
- `pay_way` - ENUM('alipay','wechat','card','wx_pub','wx_app') DEFAULT 'alipay'（支付方式）
...
- `login_user` - VARCHAR(50) UNIQUE NOT NULL（登录用户名，唯一）
- `login_pwd` - VARCHAR(255) NOT NULL（登录密码）
- `created_at` - DATETIME DEFAULT CURRENT_TIMESTAMP（创建时间）

---

## 15. merchant_balance（商家余额表）

**字段列表：**
- `merchant_id` - BIGINT UNSIGNED PRIMARY KEY（商家ID，主键）
- `balance` - DECIMAL(10,2) NOT NULL DEFAULT 0（余额）
- `bank_name` - VARCHAR(100) DEFAULT ''（银行名称）
- `bank_account` - VARCHAR(50) DEFAULT ''（银行账号）
- `updated_at` - DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP（更新时间）

**外键：**
- `merchant_balance_ibfk_1`: merchant_id → merchants(id)
...
## 19. order_split（订单拆分表）

**字段列表：**
- `id` - BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY（主键）
- `order_number` - VARCHAR(50) NOT NULL（订单号）
- `item_type` - ENUM('merchant','pool') NOT NULL（项目类型：商家/资金池）
- `amount` - DECIMAL(10,2) NOT NULL（金额）
- `pool_type` - VARCHAR(20) NULL（资金池类型）
- `created_at` - DATETIME DEFAULT CURRENT_TIMESTAMP（创建时间）

---

## 20. merchant_statement（商家对账单表）
...
## 25. merchant_balance_income（商家余额收入表）

**字段列表：**
- `id` - BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY（主键）
- `merchant_id` - BIGINT UNSIGNED NOT NULL（商户ID）
- `amount` - DECIMAL(10,2) NOT NULL（金额）
- `created_at` - DATETIME DEFAULT CURRENT_TIMESTAMP（创建时间）
...
- **主要业务模块：**
  ...
  - 订单模块：orders, order_items, order_split, refunds, alert_order
  - 财务模块：finance_accounts, account_flow, withdrawals, merchant_balance, merchant_statement, merchant_balance_income, merchant_withdraw
