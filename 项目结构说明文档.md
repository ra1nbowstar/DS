...
**类**:
- `AddressManager`: 地址管理器类
  - `add_address(user_id: int, address_data: dict)`: 添加收货地址
  - `list_addresses(user_id: int)`: 查询地址列表
  - `set_default(addr_id: int, user_id: int)`: 设置默认地址
  - `delete_address(addr_id: int, user_id: int)`: 删除收货地址
...
- `GET /address/{user_id}` - 查询用户地址列表
- `POST /address/set_default` - 设置默认地址
- `DELETE /address/{addr_id}` - 删除收货地址
