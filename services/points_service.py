...
def add_points(user_id: int, points_type: str, amount: Decimal, reason: str = "系统赠送"):
    """积分变动：写流水 + 更新余额
    
    Args:
        user_id: 用户ID
        points_type: 积分类型，'member' 或 'merchant'
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            # 2. 写流水
            cur.execute(
                "INSERT INTO points_log(user_id, points_type, change_amount, balance_after, reason) VALUES (%s,%s,%s,%s,%s)",
                (user_id, points_type, amount, balance_after, reason)
            )
